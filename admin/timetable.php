...
$action='';
$action=$_GET['action'];
define("PAGE",'timetable.php');
define("TITLE",'Time Table');
define("DBTAB",'timetable');
define("UPLOAD",'../upload/timetable/');
	...
	if($_FILES['icon']['name'] != '')
		{
			...
			if($ext != '' && !in_array($ext,array('jpeg','jpg','png','gif','pdf','docx','doc')))
			{
				$stat["error"] = "Only JPG, GIF & PNG, PDF, DOCX files are allowed.";
			}
		}
