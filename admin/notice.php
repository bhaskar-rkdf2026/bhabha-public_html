...
$action='';
$action=$_GET['action'];
define("PAGE",'notice.php');
define("TITLE",'Notice');
define("DBTAB",'notice');
define("UPLOAD",'../upload/notice/');
...
if(isset($_POST['submit']))
{
	if($_FILES['icon']['name'] != '')
		{
			...
			if($ext != '' && !in_array($ext,array('jpeg','jpg','png','gif','pdf','docx')))
			{
	...
	if($_REQUEST['action']=="add" && count($stat) == 0)	
	{
		$data = Array(
					...
					"is_examination" => $_POST['is_examination']
					 );
            ...
            <div class="card m-b-20">
              ...
              <div style="margin-left:10px; margin-right:10px;"> <?php echo msg($stat);?></div>
                ...
                <form action="" method="post" enctype="multipart/form-data">
                  ...
                  <div class="form-group col-xs-12">
                    <label>IS Examination </label>
                    <input type="checkbox" name="is_examination" value="1" <?php if($action=="edit"){
					  if($aryData['is_examination']==1){?> checked="checked" <?php }}?>/>
                  </div>
                  <div class="form-group col-xs-12">
                    <label>File</label>
