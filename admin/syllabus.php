...
$action='';
$action=$_GET['action'];
define("PAGE",'syllabus.php');
define("TITLE",'Scheme & Syllabus');
define("DBTAB",'syllabus');
define("UPLOAD",'../upload/syllabus/');
	...
	if($_FILES['icon']['name'] != '')
		{
			...
			if($ext != '' && !in_array($ext,array('pdf','doc','docx')))
			{
				$stat["error"] = "Only PDF, DOC & docx Files are allowed.";
			}
		}
...
if(is_array($course) && count($course)>0)
          {
              ...
              { 
                ?>
                      <option value="<?php echo $icourse['id']?>"
                <?php if($action=="edit"){
						 if($icourse['id']==$aryData['course']){?> selected="selected" <?php }}?><?php if($action=="add"){ if($icourse['id']==$_POST['course']){?> selected="selected" <?php }}?>
                
                
                >
                      <?php echo $icourse['course']?>
                      </option>
                      <?php
			  }
		  }?>
                  ...
                  <div class="form-group col-xs-12">
                    ...
                    <select name="branch" id="branch" class="form-control">
                      ...
                      <?php
	$db->where('course',$aryData['course']);
...
if(is_array($branch) && count($branch)>0)
          {
              ...
              { 
                ?>
                      <option value="<?php echo $ibranch['id']?>"
                <?php if($action=="edit"){
						 if($ibranch['id']==$aryData['branch']){?> selected="selected" <?php }}?><?php if($action=="add"){ if($ibranch['id']==$_POST['branch']){?> selected="selected" <?php }}?>
                
                
                >
                      <?php echo $ibranch['branch']?>
                      </option>
                      <?php
			  }
		  }?>
              ...
              foreach($aryData as $iList)
              {
                ?>
                      <tr>
						...
						<td><?php
						$db->where('id',$iList['branch']);
						$branch = $db->getOne('branch');
						
						echo $branch['branch'];?></td>
                        <td><?php echo ucfirst($iList['heading']);?></td>
