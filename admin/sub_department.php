...
$action='';
$action=$_GET['action'];
define("PAGE",'sub_department.php');
define("TITLE",'Sub Department');
define("DBTAB",'sub_department');
              ...
              <div class="card-body">
                ...
                <div class="form-group col-xs-12">
                    ...
                    <option value="">Select Institute</option>
                         ...
                         <?php
		$institute = $db->get('institute');
         if(is_array($institute) && count($institute)>0)
          {
              foreach($institute as $iinstitute)
              {
                ?>
                <option value="<?php echo $iinstitute['id']?>"
                <?php if($action=="edit"){
						 if($iinstitute['id']==$aryData['institute']){?> selected="selected" <?php }}?><?php if($action=="add"){ if($iinstitute['id']==$_POST['institute']){?> selected="selected" <?php }}?>
                
                
                ><?php echo $iinstitute['institute_name']?></option>
                <?php
			  }
		  }?>
                    </select>
                  ...
                  <div class="form-group col-xs-12">
                    <label>Content Or Data</label>
                    <textarea name="content" class="form-control ckeditor" rows="3" ><?php if($action=="edit"){echo $aryData['content'];}else{echo $_POST['content'];}?>
</textarea>
                  </div>
                  ...
                  <div class="form-group col-xs-12">
                    <label>Status</label>
                    <input type="checkbox" name="status" value="1" <?php if($action=="edit"){ if($aryData['status']==1){ echo "checked";}}?>>
                  </div>
