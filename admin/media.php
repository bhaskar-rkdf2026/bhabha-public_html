...
$action='';
$action=$_GET['action'];
define("PAGE",'media.php');
define("TITLE",'Media Library');
define("DBTAB",'media');
define("UPLOAD",'../upload/media/');
	...
	if($_FILES['icon']['name'] != '')
		{
			...
			if($ext != '' && !in_array($ext,array('jpeg','jpg','png','gif','pdf','doc','docx','xls','xlsx')))
			{
				$stat["error"] = "Only Image PDF DOC & XLS Files are allowed.";
			}
		}
              ...
              foreach($aryData as $iList)
              {
                ?>
                      <tr>
                        ...
                        <td><a href="<?php echo URL_ROOT.'upload/media/'.$iList['image']?>" target="_blank"><?php echo URL_ROOT.'upload/media/'.$iList['image']?></a></td>
